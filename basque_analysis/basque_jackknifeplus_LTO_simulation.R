...
simulation_results <- foreach(mc.run = 1:mc.samples,
      .packages = c('Synth','optimx','rgenoud')) %dopar% {
        set.seed(mc.run)
        ...
        for (tu.index in 1:length(new.index)) {
          ...
          coverages.over.time = as.integer((true.values$gdpcap <= ci.upper) & (true.values$gdpcap >= ci.lower))
        ...
        res = list(coverages, ci.lengths, power.over.time, RMSPE.power,p.values.per.dataset)
        names(res) = c("coverages","ci.lengths","power.over.time","RMSPE.power","p.values.per.dataset")
        res
      }
...
sim_results_file = sprintf("Results/basque_power_analysis_LTO_jackknife_tau%.1f_alpha%.2f_N%d.RData",
