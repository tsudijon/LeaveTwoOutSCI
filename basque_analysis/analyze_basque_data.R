...
for (j in 1:length(taus)){
  tau = taus[j]
  data.filename <- sprintf("Results/SyntheticControls/basque_power_analysis_loo_tau%.1f_alpha%.2f_N%d.RData",
...
for (j in 1:length(taus)){
  tau = taus[j]
  data.filename <- sprintf("Results/SyntheticControls/basque_power_analysis_LTO_jackknife_tau%.1f_alpha%.2f_N%d.RData",
                           tau, alpha, size.resampled.dataset)
  d <- loadRData(data.filename)
...
random.powers.over.time.Placebo  =  array(0,dim = c(length(taus), 43,mc.samples))
random.power.mean.over.time.Placebo  = matrix(0,length(taus), 43)
random.power.std.over.time.Placebo  = matrix(0,length(taus), 43)

for (j in 1:length(taus)){
  tau = taus[j]
  data.filename <- sprintf("Results/SyntheticControls/basque_power_analysis_random_placebo_test_tau%.1f_alpha%.2f_N%d.RData",
                           tau, alpha, size.resampled.dataset)
  d <- loadRData(data.filename)
