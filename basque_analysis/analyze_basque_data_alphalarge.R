...
plotname = "basque_largealpha_RMSPE_N15_alpha0.10.png"
dev.print(png, plotname, width = 500, height = 300)
...
ggplot(df2, aes(x=as.factor(time), y=value, fill=variable)) +
  ...
  geom_errorbar(aes(ymin=value-sd, ymax=value+sd), width = 0.2,
                position=position_dodge(.9)) +
  scale_fill_discrete(labels=c('Exact Placebo','Placebo', 'Naive LTO', 'LTO'), name = "Method") +
    ggtitle(sprintf("Basque Data: Effect Size: %.1f, N = 15, alpha = 0.1",taus[idx])) +
    theme(plot.title = element_text(hjust = 0.5)) +
    xlab("Effect Sizes (standard deviations)") + 
    ylab("Power") +
    scale_x_discrete(labels= 1994:1998)
