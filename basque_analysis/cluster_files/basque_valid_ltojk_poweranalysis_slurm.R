      ...
      for (tu.index in 1:length(new.index)) {
        ...
        for (t in 1:time.periods) {
          ...
          p.values.over.time[t] = 1/(length(new.index.minus.treated)) + 
            sum(LTO.residual.on.newpoint[,t] <= LTO.residuals[,t])/(0.5*length(new.index.minus.treated)^2)
        ...
        RMSPE.pvalue.dataset[tu.index] = 1/(length(new.index.minus.treated)) + 
          sum( LTO.RMSPEs >= LTO.RMSPEs.on.newpoint)/(0.5*length(new.index.minus.treated)^2) 
...
sim_results_file = sprintf("~/Results/SyntheticControls/basque_valid_power_analysis_LTO_jackknife_tau%.1f_alpha%.2f_N%d.RData",
