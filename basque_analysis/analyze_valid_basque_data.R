...
for (j in 1:length(taus)){
  tau = taus[j]
  data.filename <- sprintf("Results/SyntheticControls/basque_valid_power_analysis_LTO_jackknife_tau%.1f_alpha%.2f_N%d.RData",
...
ggplot(df2, aes(x=as.factor(taus), y=value, fill=variable)) +
  ...
  ggtitle("Basque Data: RMSPE, N = 15, alpha = 0.1") +
...
plotname = sprintf("../Figures/basque_valid_noinexact_RMSPE_N%d_alpha%.2f.pdf", size.resampled.dataset,alpha )
...
ggplot(df2, aes(x=as.factor(time), y=value, fill=variable)) +
  ...
  geom_errorbar(aes(ymin=value-sd, ymax=value+sd), width = 0.2,
                position=position_dodge(.9))+ 
  scale_fill_discrete(labels=c('Placebo', 'LTO', 'Powered LTO')) +
  ggtitle(sprintf("Basque Data: Effect Size: %.1f, N = 15, alpha = 0.1",taus[idx])) +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Year") + 
  ylab("Power")+
  scale_x_discrete(labels= 1955+time)

plotname = sprintf("../Figures/basque_valid_noinexact_overtime_N%d_alpha%.2f_tau%.2f.pdf", size.resampled.dataset,alpha,taus[idx] )
dev.print(pdf,plotname)
